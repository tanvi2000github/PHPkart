...
function thumbnail($image, $width, $height) {
	...
	if($image[0] != "/") { // Decide where to look for the image if a full path is not given
		if(!isset($_SERVER["HTTP_REFERER"])) { // Try to find image if accessed directly from this script in a browser
			$image = realpath($_SERVER['DOCUMENT_ROOT']).implode("/", (explode('/', $_SERVER["PHP_SELF"], -1)))."/".$image;
		} else {
			$image = $image;
		}
	} else {
		$image = realpath($_SERVER['DOCUMENT_ROOT']).$image;
	}
	...
	$temp_image = imagecreatetruecolor($width, $height);
          imageantialias($temp_image,true);
          imagealphablending($temp_image, false);
          imagesavealpha($temp_image,true);
          $transparent = imagecolorallocatealpha($temp_image, 255, 255, 255, 0);
          for($x=0;$x<$width;$x++) {
            for($y=0;$y<$height;$y++) {
              imageSetPixel( $temp_image, $x, $y, $transparent );
            }
          }	
